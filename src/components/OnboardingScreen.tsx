...
const ZODIAC_SIGNS = [
  ...
  { name: 'Capricorne', symbol: '♑', start: [12, 22], end: [1, 19] },
...
const getZodiacSign = (dateString: string) => {
  ...
  return ZODIAC_SIGNS.find(sign => {
    ...
    return false;
  }) || ZODIAC_SIGNS[8]; // Default to Capricorne if edge case (Dec 22 - Jan 19)
};
...
const OnboardingScreen = () => {
  ...
  useEffect(() => {
    ...
    if (sign) setZodiac({ name: sign.name, symbol: sign.symbol });
  }, [birthDate]);
    ...
    <div className="h-screen w-full flex flex-col p-6 sm:p-8 pt-12 sm:pt-16 overflow-hidden bg-black">
      ...
      <div className="flex gap-2 mb-8 sm:mb-12 shrink-0">
        {Array.from({ length: totalSteps }).map((_, i) => (
          <div 
            key={i} 
            className={`h-1 flex-1 rounded-full transition-all duration-500 ${i + 1 <= step ? 'gradient-premium' : 'bg-white/10'}`} 
          />
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar min-h-0">
        <AnimatePresence mode="wait">
          {step === 1 && (
            ...
            >
              <div className="mb-6 sm:mb-8">
                <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-3">Vos plus belles photos.</h2>
                <p className="text-white/50 text-base sm:text-lg leading-relaxed">
                  Les profils avec au moins 2 photos ont <span className="text-white font-semibold">3x plus de matches</span>.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3 p-4 glass rounded-[20px] sm:rounded-[24px] border border-white/5 mb-4">
                ...
                <p className="text-[10px] sm:text-[11px] text-white/40 leading-tight">
                  ...
                  Utilisez des photos lumineuses et évitez les lunettes de soleil.
                </p>
              </div>
            </motion.div>
          )}
          ...
          {step === 2 && (
            ...
            >
              <div className="mb-4">
                <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-2">Qui êtes-vous ?</h2>
                <p className="text-white/50">Dites-nous en un peu plus sur vous.</p>
              </div>
              
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-white/30 ml-4">Prénom</label>
                  <input type="text" placeholder="Ex: Alex" className="w-full glass rounded-[20px] sm:rounded-[24px] p-4 sm:p-5 outline-none focus:border-pink-500/50 transition-colors" />
                </div>
                
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-white/30 ml-4">Nom</label>
                  <input type="text" placeholder="Ex: Dupont" className="w-full glass rounded-[20px] sm:rounded-[24px] p-4 sm:p-5 outline-none focus:border-pink-500/50 transition-colors" />
                </div>

                <div className="space-y-2 relative">
                  <label className="text-xs font-bold uppercase tracking-widest text-white/30 ml-4">Date de naissance</label>
                  <input 
                    type="date" 
                    value={birthDate}
                  ...
                  {/* Discrete Zodiac Display */}
                  <AnimatePresence>
                    {zodiac && (
                      <motion.div 
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        className="absolute right-4 top-[42px] flex items-center gap-2 px-3 py-1.5 glass rounded-full border border-pink-500/20"
                      >
                        <span className="text-lg">{zodiac.symbol}</span>
                        <span className="text-[10px] font-bold uppercase tracking-wider text-pink-400">{zodiac.name}</span>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
          ...
          {step === 3 && (
            ...
            >
              <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">Où êtes-vous ?</h2>
              <div className="space-y-6">
                ...
                <div className="relative">
                  <input type="text" placeholder="Rechercher une ville" className="w-full glass rounded-[20px] sm:rounded-[24px] p-4 sm:p-5 pl-12 outline-none" />
            ...
            >
              ...
              <div className="absolute inset-0 pointer-events-none overflow-hidden">
                {[...Array(12)].map((_, i) => (
                  ...
                  >
                    {i % 3 === 0 ? (
                      <ICONS.Star className="text-yellow-400" size={12 + Math.random() * 12} />
                    ) : i % 3 === 1 ? (
                      <ICONS.Heart className="text-pink-500" size={12 + Math.random() * 12} />
                    ) : (
              ...
              <motion.div
                ...
                className="w-full max-w-[280px] p-4 glass rounded-[32px] border border-white/10 flex items-center gap-4 text-left"
              >
                ...
                <div>
                  <h4 className="text-sm font-bold text-white">Boost de Bienvenue</h4>
                  <p className="text-[10px] text-white/40">Votre profil est mis en avant pendant 30 minutes !</p>
                </div>
              </motion.div>
            </motion.div>
          )}
